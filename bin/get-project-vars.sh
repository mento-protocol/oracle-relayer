...
set_project_id() {
	...
	if [[ ${chain} != "celo-sepolia" && ${chain} != "celo" && ${chain} != "monad-testnet" && ${chain} != "monad" ]]; then
		echo "Error: Terraform Workspace must be 'celo', 'celo-sepolia', 'monad-testnet', or 'monad'."
	...
	project_name=$(awk '/variable "project_name"/{f=1} f==1&&/default/{print $3; exit}' ./infra/variables.tf | tr -d '",')-${chain}
	printf ' \033[1m%s\033[0m\n' "${project_name}"
...
cache_values() {
	...
	printf " - Chain (Terraform Workspace)"
	chain=$(terraform -chdir=infra workspace show)
	printf ' \033[1m%s\033[0m\n' "${chain}"
	...
	function_name=$(awk '/variable "function_name"/{f=1} f==1&&/default/{print $3; exit}' ./infra/variables.tf | tr -d '",')-${chain}
	printf ' \033[1m%s\033[0m\n' "${function_name}"
	...
	printf " - Pubsub Topic:"
	topic_name=$(awk '/variable "pubsub_topic"/{f=1} f==1&&/default/{print $3; exit}' ./infra/variables.tf | tr -d '",')-${chain}
	printf ' \033[1m%s\033[0m\n' "${topic_name}"
...
main() {
	...
	if [[ ${cache_loaded} -eq 0 ]]; then
		if [[ ${VERBOSE:-0} -eq 1 ]]; then
			...
			env_text="Chain: ${chain} / Project ID: ${project_id}"
			...
			printf "\n"
			printf "+%s+\n" "${border}"
			printf "| \033[1mChain:\033[0m %s / \033[1mProject ID:\033[0m %s" "${chain}" "${project_id}"
